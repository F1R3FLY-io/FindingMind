...
GitHub Pages serves static files, so there is no server to receive a POST. The comments
page at `/read/contact.html` posts to a third-party relay that forwards to
`f1r3fly.ceo@gmail.com`. It ships **unconfigured** and needs one edit before it works.

**FormSubmit** (no account needed) is the default:

1. Load the page once with the address in the action, submit it, and confirm from the
   email FormSubmit sends you.
2. They give you a random hash. Put **the hash, not the address** into
   `_build/contact.py` as `FORM_ENDPOINT`, then rebuild.

That second step matters. A `mailto:` or a bare address in public HTML is scraped within
days, and `f1r3fly.ceo@gmail.com` is not an address you want on spam lists — the hash
routes to the same inbox without exposing it.

**Web3Forms** is the alternative: sign up, get an access key, set
`FORM_ENDPOINT = "https://api.web3forms.com/submit"` and add the key as a hidden field.
Better spam filtering; the key is public by design.

Either way, test the form after deploying and check it lands in the inbox rather than
Gmail's spam folder — relayed mail from an unfamiliar sender often does the first time.
Marking one as "not spam" fixes it permanently.
