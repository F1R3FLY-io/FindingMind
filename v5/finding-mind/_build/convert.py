...
B = Book()
MATH_MACROS = set()
TEXT_MACROS = {}          # name -> (nargs, body); expanded in the source, not by KaTeX

# a macro whose body does any of these is a DOCUMENT macro, not a maths macro.
# handing \chSci to KaTeX renders "Chapter \refch:sci\xspace" instead of a link.
TEXT_TRIGGERS = ("\\ref", "\\cite", "\\pageref", "\\xspace", "\\footnote",
                 "\\index", "\\autoref", "\\eqref")
...
def inline(s, ctx):
    ...
    # list-spacing knobs: \itemsep0pt etc. take a bare dimension, not a group
    s = re.sub(r"\\(?:itemsep|parsep|topsep|parskip|itemindent|labelsep|leftmargin)"
               r"\s*=?\s*-?[\d.]*\s*(?:pt|em|ex|mm|cm|in|\\[a-zA-Z]+)?", "", s)
    s = s.replace("\\\\", "<br>")
    s = re.sub(r"\\newline\b", "<br>", s)
    s = re.sub(r"~", "\u00a0", s)
    s = re.sub(r"\\,|\\;|\\!|\\:", " ", s)
...
# wrappers that only scale their argument. HTML has no equivalent and needs none:
# keep the content, drop the box. name -> number of leading arguments to discard.
UNWRAP = {"fitwidth": 0, "resizebox": 2, "scalebox": 1, "rotatebox": 1,
          "adjustbox": 1, "makebox": 1, "framebox": 1, "parbox": 1,
          "mbox": 0, "raisebox": 1, "textnormal": 0, "protect": None}


def unwrap_boxes(t):
    """\\fitwidth{<table>} -> <table>. The generic inline cleanup cannot do this
    because the argument contains braces of its own."""
    for name, skip in UNWRAP.items():
        if skip is None:
            t = t.replace("\\" + name, "")
            continue
        pat = "\\" + name
        ...
        while True:
            ...
            if not ok:
                out.append(t[i:k]); i = k; continue
            inner, k3 = take_group(t, k)
            if k3 == k:
                out.append(t[i:k]); i = k; continue
            out.append(t[i:j]); out.append(inner)
            i = k3
        t = "".join(out)
    return t


def expand_text_macros(t):
    """Substitute document macros (\\chSci -> Chapter~\\ref{ch:sci}) in the source, so
    the reference machinery sees a real \\ref instead of an opaque token."""
    if not TEXT_MACROS:
        return t
    names = sorted(TEXT_MACROS, key=len, reverse=True)
    ...
    for _ in range(4):
        ...
        for m in pat.finditer(t):
            if m.start() < i:
                continue
            nargs, body = TEXT_MACROS[m.group(1)]
            j = m.end()
            args = []
            ...
            rep = rep.replace("\\xspace", "")
            out.append(t[i:m.start()]); out.append(rep)
            i = j
            hit = True
        out.append(t[i:])
        t = "".join(out)
...
def convert_file(path, chapnum, url, is_front=False):
    ...
    t = unwrap_boxes(expand_text_macros(strip_comments(raw)))
...
def parse_macros():
    ...
    for m in re.finditer(r"\\(?:re)?newcommand\{?\\([a-zA-Z]+)\}?(?:\[(\d)\])?(?:\[[^\]]*\])?\s*\{", pre):
        ...
        if any(x in body for x in ["\\fontfamily", "\\selectfont", "\\makebox",
                                   "\\resizebox", "\\scalebox", "\\adjustbox",
                                   "\\parbox", "\\includegraphics", "\\ifdim",
                                   "\\item", "\\par", "\\vspace", "\\noindent",
                                   "\\begin", "\\headrulewidth", "\\centering"]):
            continue
        if any(x in body for x in TEXT_TRIGGERS):
            TEXT_MACROS[name] = (nargs, body)
            continue
        macros["\\" + name] = body
