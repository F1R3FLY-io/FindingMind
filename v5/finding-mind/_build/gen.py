...
from contact import contact_html, CONTACT_EMAIL
...
def main():
    ...
    # build check: any LaTeX control sequence surviving into visible text is a bug
    stray = {}
    for e in items:
        h = e.get("body", "")
        h = re.sub(r"<(script|pre|style)\b.*?</\1>", "", h, flags=re.S)
        h = re.sub(r"\\\((?:[^\\]|\\(?!\)))*\\\)", "", h)
        h = re.sub(r"\\\[(?:[^\\]|\\(?!\]))*\\\]", "", h)
        h = re.sub(r"<[^>]+>", "", h)
        for m in re.finditer(r"\\[a-zA-Z]{2,}", h):
            stray[m.group(0)] = stray.get(m.group(0), 0) + 1
    if stray:
        print("STRAY LATEX IN VISIBLE TEXT:",
              ", ".join("%s x%d" % (k, v) for k, v in
                        sorted(stray.items(), key=lambda x: -x[1])[:10]))
    else:
        print("stray latex: none")
...
def railnav(nav, current=None):
    ...
    out.append('<a class="rail-util" href="ledger.html">The ledger</a>'
               '<a class="rail-util" href="contact.html">Comments</a>')
    for n in nav:
        if n["t"] == "act":
...
source.</p></header>
<p class="tocutil"><a href="ledger.html">The ledger</a> \u00b7
<a href="contact.html">Send a comment</a></p>
...
def write_contact():
    open(os.path.join(READ, "contact.html"), "w").write(
        page("Comments \u00b7 Finding Mind", "plainpage", contact_html(), root="../",
             desc="Send a comment, correction or objection to the author of Finding Mind."))
...
<section class="apparatus">
  ...
  <ul class="feat">
    ...
    <li><h3><a href="read/ledger.html">The ledger</a></h3><p>Every numbered result in one
...
<footer class="foot">
  <div class="col">
    <p>New chapters and essays appear as they are written.
       <a href="https://f1r3fly.substack.com/">Follow along</a>, or
       <a href="read/contact.html">send the author a comment</a> \u2014 corrections
       and objections are the point of publishing a draft.</p>
  </div>
</footer>
"""
